...
(function () {
  ...
  function _normalizeWWMText(s) {
    ...
    // 先頭 `|` も欠けた参照メタ列 `780|#C|149` (2026-07-29 実測: 304.tier5 の vi で実在)。
    // 数字の直後に `|` が続く形だけを対象にするので `4.6%` 等の素の数値には当たらない
    s = s.replace(/\d{2,}\|[0-9#A-Z|]+/g, '');
    ...
    m.innerHTML = `
      <div class="wwm-modal wwm-modal-square wwm-cmp-modal-a">
        ...
        <div class="wwm-modal-header">
          <h2><span class="wwm-cmp-title-ja" data-i18n="xinfaTitleJa" data-kaisho="xinfaTitleJa">${_T.xinfaTitleJa||'心法対照'}</span><span class="wwm-cmp-seal">心</span></h2>
