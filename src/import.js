  ...
  m.innerHTML = `
    <div class="wwm-modal wwm-modal-square wwm-tool-modal">
      ...
      <div class="wwm-modal-header">
        <h2><span class="wwm-tool-title-ja" data-i18n="${titleKey}" data-kaisho="${titleKey}">${(window.T && T[titleKey]) || titleKey}</span>${opts?.seal ? `<span class="wwm-tool-seal">${opts.seal}</span>` : ''}</h2>
...
function openSetupModal() {
  ...
  const m = _createModal('wwmSetupModal', 'importSetupTitle', '<div id="wwmSetupBody"></div>', 'https://www.wherewindsmeetgame.com/pc/qt/20251203102905/data/base_school/images/673325b5e3e9f9f38a72b8baeazshLYQ05.png', { seal: '取' });
  ...
  function renderIntro() {
    ...
    // v3 (2026-06-22): wrapper bookmarklet = 短文 fetch+eval 化 = 本体 (bm-loader.js) 更新で全 user 自動追従。
    // 「これが最後の再登録」 = 以降 bookmarklet 改修ではユーザー再登録不要。
    // calcUrl + BM_VERSION を window 経由で loader に渡す (JSON.stringify で string injection 対策)。
    // new Function() の source = 自ドメイン (wwm-metrics.pages.dev) bm-loader.js = 信頼前提 (HTTPS pinned)。
    const loaderUrl = calcUrl + 'bm-loader.js';
    const _cuJ = JSON.stringify(calcUrl);
    const _luJ = JSON.stringify(loaderUrl);
    const bmSrc = "(async()=>{try{window.__WWM_CALC_URL=" + _cuJ + ";window.__WWM_BM_VERSION=" + WWM_BM_VERSION + ";const r=await fetch(" + _luJ + "+'?t='+Date.now());if(!r.ok)throw new Error('HTTP '+r.status);new Function(await r.text())();}catch(e){alert('WWMetrics 本体取得失敗: '+e.message);}})();";
...
async function openPreviewModal(data, importedAt, savedState) {
  ...
  const m = _createModal('wwmPreviewModal', 'importPreviewTitle', '<div id="wwmCardBody"></div>', 'https://www.wherewindsmeetgame.com/pc/qt/20251203102905/data/base_school/images/673325b5e3e9f9f38a72b8baeazshLYQ05.png', { seal: '覧' });
...
const _STAT_LABELS_PROXY = new Proxy({}, {
  get(_, k) {
    ...
    if (window.WWM_DS) {
      ...
      // vi 時 = 短縮辞書 (game.json.stat_short、 2026-07-01 統合) 優先。 武具対照 modal + import preview の文字切れ救済。
      // miss 時は通常 game.json.stat (公式表記) fallback、 真実源不変
      if (L === 'vi') {
