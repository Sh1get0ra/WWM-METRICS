  ...
  function _shareBuildUrl(opts) {
    ...
    m.innerHTML = `
      <div class="wwm-modal wwm-modal-wide wwm-tool-modal wwm-share-modal-b">
        ...
        <div class="wwm-modal-header">
          <h2><span class="wwm-tool-title-ja" data-kaisho="shareTitle">${T.shareTitle ?? '飛簡'}</span><span class="wwm-tool-title-en">SHARE</span><span class="wwm-tool-seal">飛</span></h2>
        ...
        <div class="wwm-tool-modal-footer" id="wwmShareFooter"><div id="wwmShareMsg" style="font-size:12px;color:var(--text-kami-gold);min-height:16px;"></div></div>
      </div>`;
    ...
    function _buildUrlPane(pane) {
      pane.innerHTML = `${_bgIconHtml}
        <div style="font-size:13px;color:var(--text-kami-gold);font-weight:700;letter-spacing:0.12em;margin-bottom:6px;">${(window.T?.shareSect1Heading) ?? '▍ビルド共有'}</div>
        <p style="font-size:12px;color:var(--text-kami-fg);opacity:0.92;margin:0 0 10px;line-height:1.6;">${(window.T?.shareSect1Desc) ?? ''}</p>
    ...
    function _buildObsPane(pane) {
      pane.innerHTML = `${_bgIconHtml}
        <div style="font-size:13px;color:var(--text-kami-gold);font-weight:700;letter-spacing:0.12em;margin-bottom:6px;">${(window.T?.shareSect2Heading) ?? '▍OBS 配信用 URL'}</div>
        <p style="font-size:12px;color:var(--text-kami-fg);opacity:0.92;margin:0 0 8px;line-height:1.6;">${(window.T?.shareSect2Desc) ?? ''}</p>
        ...
        <details style="margin:0 0 10px;font-size:12px;color:var(--text-kami-fg);">
          <summary style="cursor:pointer;color:var(--text-kami-gold);letter-spacing:0.05em;font-weight:700;">${(window.T?.shareObsSetup) ?? 'OBS への設定方法'}</summary>
        ...
        <div style="display:flex;align-items:center;gap:12px;font-size:12px;color:var(--text-kami-fg);margin-bottom:8px;flex-wrap:wrap;">
          ...
          <label style="display:flex;align-items:center;gap:6px;">
            ...
            <span id="wwmObsOpacityVal" style="font-family:var(--f-latin);color:var(--text-kami-gold);min-width:36px;">${initOp}%</span>
          </label>
        </div>
        <div style="display:flex;align-items:center;gap:12px;font-size:12px;color:var(--text-kami-fg);margin-bottom:8px;flex-wrap:wrap;">
        ...
        <div id="wwmSharePreviewWrap" style="display:none;margin-top:12px;">
          <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px;">
            <div style="font-size:11px;color:var(--text-kami-gold);font-weight:700;letter-spacing:0.1em;">${(window.T?.sharePreviewTitle) ?? 'プレビュー'}</div>
            <label style="font-size:11px;color:var(--text-kami-dim);display:flex;align-items:center;gap:6px;">${(window.T?.sharePreviewScaleLabel) ?? '縮尺'}
              <input type="range" id="wwmSharePreviewScale" min="30" max="100" step="5" value="50" style="width:120px;accent-color:var(--gold);">
              <span id="wwmSharePreviewScaleVal" style="font-family:var(--f-latin);color:var(--text-kami-gold);min-width:36px;">50%</span>
            </label>
          </div>
    ...
    function _buildCardPane(pane) {
      ...
      pane.innerHTML = _bgIconHtml + '<p style="color:var(--text-kami-dim);padding:20px;">...</p>';
    }
