...
const TRANSLATIONS = {
  ja: {
    ...
    ocrHelpTitle:'スクショ取込ガイド', ocrHelpExample:'撮影例: 装備詳細画面 (値が右端に並ぶ画面)', ocrHelpMockLv:'装備レベル', ocrHelpMockAtk:'外功攻撃', ocrHelpStep1:'ゲーム内で装備詳細画面を開き、Win+Shift+S で定音/調律の部分を範囲スクショ', ocrHelpStep2:'武具対照を開いたまま Ctrl+V で貼付け (📷 から画像選択 / 新置列へドロップでも可)', ocrHelpStep3:'OCR 解析が始まり、装備 Lv と定音/調律を新置列へ自動書換 (行の並びは枠の種別に合わせ自動整列)', ocrHelpStep4:'読み取りが怪しい行は朱枠表示、読めなかった行は空欄 — 確認・編集すると枠が消える', ocrHelpLangNote:'スクショの言語は UI 言語と同じものを推奨 (読取失敗時は言語選択が出ます)',
...
finalScore  = statusScore + 4-set bonus (4点一式効果発動時)</pre>
  ...
  en: {
    ...
    ocrHelpTitle:'Screenshot Import Guide', ocrHelpExample:'Example: equipment detail screen (values aligned to the right)', ocrHelpMockLv:'Equip Lv', ocrHelpMockAtk:'Phys ATK', ocrHelpStep1:'Open the equipment detail screen in game and capture the affix area with Win+Shift+S', ocrHelpStep2:'With this panel open, paste with Ctrl+V (or pick an image via 📷 / drop onto the right column)', ocrHelpStep3:'OCR runs and auto-fills equipment Lv and affixes into the right column (rows are auto-arranged to match each slot type)', ocrHelpStep4:'Uncertain rows are outlined in red; unreadable rows are left blank — review and edit to clear', ocrHelpLangNote:'Use a screenshot in the same language as the UI (a language picker appears if reading fails)',
...
finalScore  = statusScore + 4-set bonus (四件套效果发动时)</pre>
  ...
  ko: {
    ...
    ocrHelpTitle:'스크린샷 가져오기 가이드', ocrHelpExample:'예시: 장비 상세 화면 (수치가 오른쪽 정렬된 화면)', ocrHelpMockLv:'장비 레벨', ocrHelpMockAtk:'외공 공격', ocrHelpStep1:'게임에서 장비 상세 화면을 열고 Win+Shift+S로 정음/조율 부분을 캡처', ocrHelpStep2:'무구대조 패널을 연 상태에서 Ctrl+V로 붙여넣기 (📷로 이미지 선택 / 오른쪽 열에 드롭도 가능)', ocrHelpStep3:'OCR 인식 후 장비 레벨과 정음/조율이 자동 입력됩니다 (행 순서는 슬롯 종류에 맞춰 자동 정렬)', ocrHelpStep4:'인식이 의심스러운 행은 붉은 테두리, 읽지 못한 행은 공란으로 표시 — 확인 후 수정하면 사라집니다', ocrHelpLangNote:'UI 언어와 같은 언어의 스크린샷을 권장합니다 (인식 실패 시 언어 선택이 표시됩니다)',
