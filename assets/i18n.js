...
const TRANSLATIONS = {
  ja: {
    ...
    ocrHelpTitle:'スクショ取込ガイド', ocrHelpExample:'撮影例: 装備詳細画面 (値が右端に並ぶ画面)', ocrHelpMockLv:'装備レベル', ocrHelpStep1:'ゲーム内で装備詳細画面を開き、Win+Shift+S で定音/調律の部分を範囲スクショ', ocrHelpStep2:'武具対照を開いたまま Ctrl+V で貼付け (📷 から画像選択 / 新置列へドロップでも可)', ocrHelpStep3:'OCR 解析が始まり、装備 Lv と定音/調律を新置列へ自動書換', ocrHelpStep4:'読み取りが怪しい行は朱枠表示 — 内容を確認し、編集すると枠が消える', ocrHelpLangNote:'スクショの言語は UI 言語と同じものを推奨 (読取失敗時は言語選択が出ます)',
...
finalScore  = statusScore + 4-set bonus (4点一式効果発動時)</pre>
  ...
  en: {
    ...
    ocrHelpTitle:'Screenshot Import Guide', ocrHelpExample:'Example: equipment detail screen (values aligned to the right)', ocrHelpMockLv:'Equip Lv', ocrHelpStep1:'Open the equipment detail screen in game and capture the affix area with Win+Shift+S', ocrHelpStep2:'With this panel open, paste with Ctrl+V (or pick an image via 📷 / drop onto the right column)', ocrHelpStep3:'OCR runs and auto-fills equipment Lv and affixes into the right column', ocrHelpStep4:'Uncertain rows are outlined in red — review and edit them to clear the outline', ocrHelpLangNote:'Use a screenshot in the same language as the UI (a language picker appears if reading fails)',
...
finalScore  = statusScore + 4-set bonus (when 4 Pieces active)</pre>
  ...
  zh: {
    ...
    errImportParse:'导入数据解析失败。请在官方工具中重新执行导入。', errSharedBuildLoad:'共享配装加载失败 — 当前显示的是你自己的数据。', errStatsBuild:'计算失败，请尝试重新导入。', errExportFail:'图片导出失败', errExportOffline:'图片导出需要联网', errShareUrlFail:'共享链接生成失败', scoreUpdateMsg:'计算数据已更新，请重新导入以反映最新分数。', scoreUpdateBtn:'重新导入', migrationMsg:'已迁移到新地址: <strong>wwm-metrics.pages.dev</strong> — 当前 (旧) 地址将于 2026-06-08 前后停用', migrationBtn:'前往新地址', importPrompt:'请点击上方「导入」按钮加载数据', importHintLabel:'从这里导入', importBtn:'导入', exportBtn:'导出', resetBtn:'重置', shareBtn:'分享', noteBtn:'笔记',
    ...
    ocrHelpTitle:'截图导入指南', ocrHelpExample:'示例: 装备详情界面 (数值右对齐的界面)', ocrHelpMockLv:'装备等阶', ocrHelpStep1:'在游戏中打开装备详情界面，用 Win+Shift+S 截取定音/调律区域', ocrHelpStep2:'保持武具对照面板打开，按 Ctrl+V 粘贴 (也可用 📷 选择图片 / 拖入右列)', ocrHelpStep3:'OCR 识别后，装备等级与定音/调律将自动填入右列', ocrHelpStep4:'识别可疑的行会显示红框 — 确认并修改后红框消失', ocrHelpLangNote:'建议使用与界面语言相同的截图 (识别失败时会出现语言选择)',
...
finalScore  = statusScore + 4-set bonus (四件套效果发动时)</pre>
  ...
  ko: {
    ...
    ocrHelpTitle:'스크린샷 가져오기 가이드', ocrHelpExample:'예시: 장비 상세 화면 (수치가 오른쪽 정렬된 화면)', ocrHelpMockLv:'장비 레벨', ocrHelpStep1:'게임에서 장비 상세 화면을 열고 Win+Shift+S로 정음/조율 부분을 캡처', ocrHelpStep2:'무구대조 패널을 연 상태에서 Ctrl+V로 붙여넣기 (📷로 이미지 선택 / 오른쪽 열에 드롭도 가능)', ocrHelpStep3:'OCR 인식 후 장비 레벨과 정음/조율이 자동 입력됩니다', ocrHelpStep4:'인식이 의심스러운 행은 붉은 테두리로 표시 — 확인 후 수정하면 사라집니다', ocrHelpLangNote:'UI 언어와 같은 언어의 스크린샷을 권장합니다 (인식 실패 시 언어 선택이 표시됩니다)',
